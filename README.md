...
| Safari · Chrome 일반 본문 | **확인** | (불필요) | 본문 입력란. 주소창은 Enter 제출 시만 제외 |
...
- 충돌을 **알고도 연** 한 글자가 18개 있습니다: `도` `이` `의` `에` `나` `시` `초` `제` `호` `개` `야` `또` `못` `님` `놈` `곧` `좀` `꽤`. 한국어 쪽 빈도가 훨씬 높다고 판단한 결과이며, 결정과 사유는 [`scripts/lexicon/mono-admit.tsv`](scripts/lexicon/mono-admit.tsv)에 한 줄씩 남아 있습니다.
  - `해`(`go`)와 `년`(`sus`)은 한때 열려 있었으나 **되돌렸습니다.** 양쪽 다 흔한 말이면 손대지 않는 것이 원칙입니다 — `go `는 영어에서 가장 흔한 동사 중 하나이고, `sus`는 결과가 욕설로 읽힐 수 있습니다. `도`(`eh`) · `또`(`Eh`)는 열려 있습니다.
  - 되돌리려면 해당 줄을 지우고 `python3 -B scripts/lexicon/make_mono_lexicon.py generate` 를 다시 돌리면 됩니다. 코드는 건드리지 않습니다.
- 아직 못 연 한 글자: `그`(`rm` 명령어) · `여`(`du` 명령어) · `두`(`en`) · `내`(`so`) · `옴`(`dha`) · `츰`(`cma`). 전부 영어 단어나 명령어와 정면으로 겹칩니다.
...
- 비밀번호·보안 입력란에서는 **의도적으로** 동작하지 않습니다.
- 주소창·URL 입력란에서는 스페이스·문장부호 경계까지만 교정하고, Enter·Tab으로 제출할 때는 교정하지 않습니다. 제출하면 페이지가 넘어가 ⌘Z로 되돌릴 수 없기 때문입니다.
- 자판 자동 교정은 **실험적** 기능입니다.
...
`앱을 가리지 않고 적용`을 처음 선택하면 모든 앱과 브라우저 웹사이트에 적용된다는 확인 창이 나타납니다. 검색 입력란은 지원하며 비밀번호·보안 입력 필드는 어느 범위에서도 제외됩니다. 주소·URL 입력란은 Enter·Tab 제출 시에만 제외됩니다.
...
| `VERSION` | 표시 버전; 기본값 `1.11` |
| `BUILD_NUMBER` | `CFBundleVersion`; 기본값 `17` |
...
|------|------|
| 소스 버전 | 1.11 (build 17) |
| 최신 공개 배포 | **v1.11** (2026-07-26, 서명·공증 완료). v1.3이 첫 공식 릴리스 |
| 개발사 | Draftup |
