...
| 한글 자판 | macOS 한국어 두벌식 |
| 적용 범위 | 자동 교정은 전체 Mac 또는 선택 앱, 기존 한글 조합 보정은 등록 앱 |
...
`전체 Mac`을 처음 선택하면 모든 앱과 브라우저 웹사이트에 적용된다는 확인 창이 나타납니다. 검색 입력란은 지원하며 비밀번호·주소·보안 입력 필드는 어느 범위에서도 제외됩니다.

### 3. 대상 앱 등록

기존 한글 조합 보정이 필요하거나 `선택한 앱만` 모드에서 앱별 자동 교정을 사용할 때 등록합니다. 자동 교정 범위를 `전체 Mac`으로 바꿔도 등록 앱의 한글 조합 설정은 계속 적용되며, 메뉴에는 조합 대상 앱과 토글이 그대로 표시됩니다. 이때 앱별 자동 교정 토글 대신 전역 범위를 사용합니다.
