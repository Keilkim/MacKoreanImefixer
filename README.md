...
- 확인된 예: **Adobe Illustrator** — 문자 도구로 캔버스에 친 글자는 교정되지 않습니다.
- 한글 조합 보정(등록 앱 기능)은 AX가 필요 없어 이런 앱에서도 동작합니다.

### 2. 어떤 앱에서 되는지 **미리 알 수 없습니다**

앱 목록의 자판자동 칸은 `동작 확인` / `미확인` / `미지원` 셋 중 하나를 보여주는데, `미확인`은 **그 앱에서 실제로 쳐 봐야** 풀립니다. 기다린다고 풀리지 않습니다.

미리 판별할 방법을 찾다가 실패했고, 그 근거를 남깁니다 — AX 트리에 텍스트 요소가 있는지는 지원 여부와 **상관이 없습니다.**

| 앱 | AX 텍스트 요소 | 자동 교정 |
|---|---:|---|
| Adobe Illustrator | 27개 | **안 됨** |
| Xcode · VS Code · 터미널 · 메시지 | 0개 | **됨** |

캔버스 텍스트는 AX 요소가 아니고, 반대로 Electron·Xcode는 포커스된 순간에만 요소를 만들기 때문입니다. `미지원`으로 판정된 앱은 옆의 **요청** 버튼으로 이슈를 남길 수 있습니다.
