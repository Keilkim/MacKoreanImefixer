...
### 1. 자동 교정이 원리적으로 불가능한 앱이 있습니다

자동 교정은 macOS 접근성(AX)으로 입력란을 읽어야 동작합니다. 화면에 글자를 **직접 그리는** 앱은 입력란이라는 것 자체가 없어서 방법이 없습니다.

- 확인된 예: **Adobe Illustrator**(문자 도구 캔버스), **한컴오피스 한글**(본문이 `AXScrollArea`라 AX에 편집 텍스트가 없음) — 자판자동이 되지 않습니다.
...
| 앱 | 자판자동 | 근거 |
|---|---|---|
| TextEdit · VS Code · Xcode · 터미널 · 메시지 | **확인** | AXTextArea 노출(포커스 시 생성 포함) |
| Safari · Chrome 일반 본문 | **확인** | 주소창 제외, 본문 입력란 |
| Adobe Illustrator | **미지원** | 문자 도구는 캔버스, AX에 본문 텍스트 없음 |
| CorelDRAW | **미지원** | 〃 |
| IntelliJ 계열 | **미지원** | 편집기 텍스트를 AX에 미노출 |
| 한컴오피스 한글(HWP) | **미지원** | 본문이 `AXScrollArea`, AX 트리 전체에 본문 텍스트 요소 0개 — 툴바 컨트롤만 노출(실측). `AXManualAccessibility`·`AXEnhancedUserInterface` 활성화도 실패 |

**조합 — 표준 편집 입력란이면 대체로 동작**하며, 앱별로 켜고 끕니다. 자체 한글 입력을 갖춘 워드프로세서(한컴 등)에서는 불필요합니다. 조합은 AX를 쓰지 않아 "미지원"으로 판정되는 경우가 드뭅니다.

`미지원`으로 판정된 앱은 옆의 **요청** 버튼으로 이슈를 남길 수 있습니다. 자판자동 지원은 앱의 AX에 달려 있어, 같은 원인을 푸는 개선은 항상 특정 앱 이름이 아니라 **공통 계층**에 들어갑니다([CONTRIBUTING.md](CONTRIBUTING.md) 참고).
