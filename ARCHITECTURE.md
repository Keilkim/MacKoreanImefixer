...
- 한국어 두벌식 상태에서 `팿미 `에 해당하는 물리 키 `vocal` 입력 → macOS 영어 hit·한국어 miss이면 `vocal `로 교체
- 후행 자모형 `해ㅐㅇ `에 해당하는 물리 키 `good` 입력 → 같은 비대칭 시스템 근거가 있으면 `good `으로 교체
- 같은 키열을 양쪽 사전이 인정하는 `재구/worn` → 현재 한국어 source 보존
- `dksehlsmsrjsep?` → `안되는건데?`
- `dkwn whgsp` → `아주 좋네`
- 숫자·지원 밖 기호가 섞인 run, ALL CAPS source, 양쪽이 모두 자연스러운 후보 → 변경하지 않음

위 단어들은 규칙을 검증하는 회귀 fixture이지 생산 코드의 전용 분기나 실행 중 순회하는 사례 목록이 아니다. 새 반례도 단어를 한 건씩 fallback에 추가하지 않고 일반 구조·양쪽 macOS 사전·안전 gate를 수정해 해결한다.

이 기능은 맞춤법 교정, 문맥 이해, AI 번역이 아니다. 현재 단어의 제한된 물리 키 흐름으로 영문 후보와 두벌식 후보를 만들고, source 보호 근거·후보 구조·작은 bootstrap fallback·macOS 로컬 언어 자원을 정해진 우선순위로 비교한다. 시스템 자원이 unavailable이어도 강한 구조 비대칭은 중간 확신으로 자동 적용할 수 있다.

## 3. 명시적인 비목표

- 문장 전체를 읽거나 의미를 추론하지 않는다.
- 맞춤법, 띄어쓰기, 영문 오타를 고치지 않는다.
- 입력 보정에는 네트워크, 외부 AI/LLM, 유료 API 또는 사용자 API 키를 사용하지 않는다. 네트워크는 서명된 공식 업데이트 확인·다운로드에만 한정한다.
...
| 기존 한글 조합 보정 | 저장된 앱별 설정을 일시 정지하고 macOS 네이티브 IME 유지 | 등록 앱 중 앱별 조합 보정을 켠 앱 |
| 대상 앱 목록·추가 UI | 숨김 | 표시 |
| 저장된 대상 앱 설정 | 유지 | 사용 |
...
새 앱의 안전한 기본값은 `한글 조합 보정 = 켬`, `자동 교정 = 끔`이다. 선택 모드에서 앱별 자동 교정을 켜면 현재 설정 규칙상 조합 보정도 함께 켜져 그 명시적으로 등록한 앱에서는 직접 조합이 활성화될 수 있다. 조합 보정을 끄면 그 앱의 자동 교정도 함께 꺼진다. `전체 Mac` 모드는 이 앱별 직접 조합 설정을 사용하지 않는다.
...
| `CorrectionNoticeController.swift` | 교정된 exact range 위의 non-activating 원문-only 클릭 칩; 기능 플래그·4초 수명·hit-test |
...
2. 처음 키에서 현재 AX 포커스가 안전한 일반 텍스트 필드인지 확인하고 포커스 토큰과 최초 커서 위치를 잡는다.
3. 글자 물리 이벤트는 앱에 그대로 전달하면서 키코드, Shift 상태와 시간만 RAM 버퍼에 넣는다. Caps Lock은 영문 자판에서 Shift와 동등하게 기록하고(화면과 대소문자를 일치시켜야 Undo가 원문을 훼손하지 않는다), 한글 자판에서는 IME 출력을 보장할 수 없어 토큰을 경계까지 무효화한다.
4. Space·`,`·`?`·`!`의 물리 keyDown은 앱에 먼저 전달한다. `.`은 1~3개까지 유예하고, 뒤에 내부 문자가 이어지면 URL·도메인 run으로 보고 전체 토큰을 폐기한다.
5. 최종 trigger 경계의 keyUp이 오면 앱과 AX 커서가 정착하도록 짧게 재시도하면서, 같은 안전한 AX 요소인지와 커서가 `원문 UTF-16 길이 + BoundarySequence UTF-16 길이`만큼 이동했는지 다시 확인한다.
6. 확인을 통과하면 앱이 이미 넣은 경계 배열을 뒤에서부터 지우고 원문을 지운 다음, 한글 Unicode 문자열과 동일 경계 배열을 순서대로 한 번 넣는다.
7. 시스템 입력 소스를 정확한 한국어 두벌식 ID로 바꾼다.
8. 6초 복원 트랜잭션을 만들고, 교정된 exact range의 문자열·좌표를 검증할 수 있으면 물리 키열로 재구성한 원문 하나만 최대 4초 표시한다.
...
`전체 Mac`과 직접 조합을 켜지 않은 자동 교정 경로에서는 macOS 한국어 두벌식 IME가 원래 물리 키를 받아 화면의 한글 조합을 전담한다. Mackor은 화면 문자열을 읽지 않고 같은 물리 키 흐름으로 원래 한글 후보와 대체 영문 후보를 재구성한다.

1. 안전한 자동 교정 필드에서 글자 물리 keyDown/keyUp을 네이티브 IME에 그대로 보내면서 키코드와 Shift 상태를 기록한다.
2. macOS IME가 marked text와 완성 음절을 정상적으로 처리하며, `전체 Mac` 모드의 Mackor은 이를 직접 조합하거나 가로채지 않는다.
...
직접 조합은 `선택한 앱만` 모드에서 사용자가 등록하고 조합 보정을 켠 앱에만 한정된다. `전체 Mac`에서는 저장된 앱별 직접 조합 설정을 정지하고 네이티브 IME를 유지한다.

### 8.3 선택 앱 직접 조합만 사용하는 경우

자동 교정이 꺼져 있어도 `선택한 앱만` 모드에서 등록 앱의 조합 보정을 명시적으로 켜면 기존 방식으로 작동한다. 자모 키의 원래 이벤트를 막고 조합 상태 머신 결과를 Unicode로 보낸다. 방향키, Escape, 비자모 키 등에서는 진행 중 조합을 확정하거나 초기화하고 앱의 원래 이벤트를 통과시킨다. Return/Enter/Tab은 v1.3부터 제출 경계로도 쓰이므로 위 표를 따른다.
...
| 입력 | 처리 |
|---|---|
| Space | Shift가 없을 때 교정 경계 |
| `.` | 1~3개까지 후행 마침표로 유예; 다음 글자·숫자·내부 기호가 오면 전체 run 폐기, 4번째 마침표도 폐기 |
| `,` | Shift가 없을 때 교정 경계 |
| `?` | Shift+`/`일 때 교정 경계; Shift를 보존해 재주입 |
| `!` | Shift+`1`일 때 교정 경계; Shift를 보존해 재주입 |
| Return / 숫자패드 Enter / Tab | 제출 경계. 교정 후보가 있으면 키를 붙잡아 원문을 교정한 뒤 그 키를 주입하고, 후보가 없으면 그대로 통과한다. 교정을 포기하더라도 키는 반드시 주입한다. `Shift+Tab`은 역방향 포커스 이동이라 제외 |
...
| 그 밖의 Shift 글자 토큰 | Shift를 물리 후보에 보존; 구조-only Latin→Korean에서는 자모가 달라지는 Shift 타건을 보수적으로 veto |
| Caps Lock이 들어간 글자 토큰 | 영문 자판에서는 Shift로 기록(전대문자는 약어 규칙이 보존). 한글 자판에서는 다음 실제 경계까지 무효화 |
...
- 각 키의 Shift 상태. Caps Lock은 영문 자판에서 Shift로 기록하고, 한글 자판에서는 토큰을 경계까지 무효화
- 토큰이 시작된 지원 입력 소스 종류
- 마지막 키 입력 시각
...
엔진은 사전 승인 경로와 구조 승인 경로를 명시적으로 분리한다. 현재 production 엔진에는 부동소수점 점수나 가산점 임계값이 없고, 승인 규칙 어느 것도 성립하지 않으면 source를 보존한다. 공통 우선순위는 다음과 같다.

1. 안전하지 않은 문맥과 지원 밖 run을 폐기한다.
2. Latin source의 물리 ALL CAPS 2자 이상을 target exact보다 먼저 보존한다.
3. source bootstrap 단어와 Korean source의 고정·전체 반복 자모 표현을 먼저 보존한다. 완성형이라는 사실만으로는 하드 veto하지 않는다.
4. 시스템 근거를 `recognized`, `notRecognized`, `unavailable`로 구분하고 authoritative source hit를 보존한다.
5. 두벌식·영어 두 문법의 충족 여부와 위반 비용을 비교해 교정 여부와 신뢰 등급을 정한다. 사전과 시스템 맞춤법 서비스는 참조하지 않는다.
6. 방향별 구조 비대칭을 평가하고, 어떤 승인 규칙도 성립하지 않은 나머지는 이유 enum과 함께 source를 보존한다.

한글 후보는 `fullyComposed`, `mixed`, `pureJamo`, `unsupported`로 나눈다. Korean→Latin에서 `pureJamo`는 영문 후보가 plausible하거나 authoritative 영어 hit가 있고, 번들 또는 authoritative 영어 target 근거도 있어야 한다. 따라서 `ㅠㅕㅅ → but`, `ㅙㅈ → how`는 허용하지만 `ㅁㄴㅇ/asd`, `ㅁㄴㅇㄹ/asdf`는 보존한다. 토큰 전체가 한 자모로 반복된 `ㄷㄷㄷ/eee`, `ㅁㅁㅁ/aaa`도 선행 보호하지만 `ㄴㄷㄷ/see`, `ㅁㅇㅇ/add`처럼 일부 double letter만 반복된 정상 영어는 사전 근거로 복구할 수 있다. `mixed`는 자모로 시작하는 `ㅗ디ㅣㅐ/hello`, `ㅊ무/can`을 교정한다. `안녕ㅎ`, `진짜ㅋㅋ`, `와ㄷㄷ`, `아ㅠ` 같은 완성 음절 prefix + 자모 suffix는 기본 보존하되 `해ㅐㅇ/good`, `챌ㄹㄷㄷ/coffee`처럼 macOS 영어 hit와 실제 한국어 miss가 함께 확인되면 이 soft veto를 넘는다. `fullyComposed`도 실제 한국어 단어의 증거로 간주하지 않으며 `팿미/vocal`처럼 영어만 인식될 때 교정하고 `재구/worn`처럼 양쪽이 인식되면 source를 보존한다. 사전을 호출하지 않은 상태와 호출했지만 사용할 수 없는 상태는 각각 `notRequested`, `unavailable`로 진단한다.

Latin→Korean은 두 경로를 쓴다. 구조-only 경로는 source 영어가 인식되지 않고, 영문 형태가 강하게 implausible하며, 한글 후보가 fully composed이고, Shift-sensitive fallback veto가 없을 때 동작한다. 별도의 authoritative 경로는 macOS 한국어 사전만 fully-composed target을 인식하면 영문 철자 heuristic과 무관하게 승인한다. 어느 쪽도 단어별 예외를 추가하지 않는다. `qlfem → 빌드`, `gksk → 하나`, `rksk → 가나`, `qkfto → 발새`, 소문자 `dhcp → 오체`가 정책상 승인될 수 있지만 `GKSK`, `DHCP`, `NASA`, `HTTP`, `XML`은 물리 ALL CAPS source라 보존한다.

시스템 근거 unavailable은 “단어가 아님”이라는 authoritative 부정이 아니다. 구조 비대칭 결과는 `mediumUnavailable`로 구분해 적용하며, 사용 가능한 시스템이 source를 인식하지 않은 구조 결과는 `mediumEvidence`, source veto를 통과한 exact/authoritative target은 `high`로 진단한다. 후보 문자열은 로그·캐시·지속 저장에 남기지 않고, 문자열 없는 reason·case·shape·availability만 진단 값으로 사용할 수 있다.

v1.3부터 Shift-sensitive 구조 veto와 ALL CAPS 하드 veto는 없다. 물리 Shift는 두벌식에서 쌍자음을 만들므로 후보에 그대로 반영되고, 전대문자 토큰은 약어 표기 관례 규칙(R-D3)이 보존한다. 이 정책의 정밀도·재현율은 `scripts/rulebench/bench.py`의 대규모 평가셋과 tune/holdout/golden 코퍼스로 회귀 검증한다.
...
- 역할이 `AXTextField` 또는 `AXTextArea`다.
...
- `AXSecureTextField` 또는 `AXSearchField` 하위 역할
- title, description, role description, placeholder 메타데이터에 다음 힌트 포함
  - `address`, `url`, `location`, `web address`, `search`
  - `주소`, `웹 주소`, `위치`, `검색`
  - `password`, `passcode`, `암호`, `비밀번호`
- 역할, 선택 범위, 포커스 요소를 확인할 수 없음
- 텍스트가 선택되어 있음

판단에 실패하면 교정하지 않는 fail-closed 정책이다. 시스템 전체 AX 요소의 메시징 타임아웃은 50ms로 제한한다.
...
첫 글자에서 안전한 AX 요소와 최초 선택 위치를 토큰으로 잡는다. 토큰 도중에는 매 글자마다 동기 AX 왕복을 하지 않고, 앱 전환·마우스 클릭·입력 소스 변경 같은 명시적 상태 변화에서 토큰을 폐기한다. Space·문장부호 후보가 생기면 물리 경계 keyDown을 먼저 통과시킨 뒤 일치하는 keyUp 뒤 20ms 간격으로 최대 세 번, 위 안전 메타데이터와 같은 AX 요소인지와 최초 위치에서 `원문 UTF-16 길이 + BoundarySequence 전체 UTF-16 길이`만큼 실제 커서가 이동했는지를 재검증한다. 어느 시도에서도 일치하지 않으면 이미 화면에 있는 원문과 경계를 수정하지 않는다.

Return·숫자패드 Enter·Tab은 제출 또는 포커스 이동이 먼저 일어나면 되돌릴 수 없으므로 후보가 있을 때만 keyDown을 차단한다. 같은 keyDown 처리 안에서 `원문 UTF-16 길이 + 이미 통과한 후행 마침표 길이` 커서를 확인하고, 마침표와 원문을 지운 뒤 교정문·마침표·제출 키 순서로 주입한다. 20ms scheduler를 사용하지 않아 다음 물리 키가 제출 키를 추월하지 못한다. 후보가 없으면 원래 keyDown/keyUp을 그대로 통과시키며 Shift+Tab은 제출 경계에서 제외한다.
...
### 12.1 Space·문장부호의 사후 교체 순서

1. 사용자가 누른 경계의 물리 keyDown을 막지 않고 대상 앱에 먼저 보낸다.
2. `BoundarySequence`의 최종 물리 trigger keyUp 뒤 안전 메타데이터, 동일 포커스와 `원문 UTF-16 길이 + 경계 배열 UTF-16 길이` 커서를 확인한다. keyUp 자체도 막지 않는다.
3. 앱에 이미 입력된 경계 배열을 역순 Backspace keyDown/keyUp으로 먼저 지운다.
4. 원문 `Character` 수만큼 Backspace keyDown/keyUp을 보낸다. 각 삭제 사이에 3ms를 둔다.
5. 대체 문자열 전체를 하나의 Unicode keyDown/keyUp으로 보낸다.
6. 3ms 뒤 사용자가 실제 누른 경계 배열의 키와 Shift 상태를 원래 순서로 한 번 복원한다.
7. TIS로 목표 입력 소스를 선택한다.

주입 이벤트에는 전용 마커가 붙는다. 물리 trigger를 선제적으로 차단하는 방식이 아니라, 원래 trigger keyDown/keyUp을 통과시킨 사후 트랜잭션에서 이미 입력된 `BoundarySequence`를 삭제하고 동일한 배열을 한 번 복원해 중복 구두점과 고아 keyUp을 피한다.

### 12.2 제출 경계의 선행 교체 순서

1. Return·숫자패드 Enter·Tab keyDown에서 실제 교정 후보가 있는지 평가한다.
2. 후보가 없으면 물리 keyDown/keyUp을 그대로 통과시킨다. Shift+Tab도 이 경로다.
3. 후보가 있으면 물리 keyDown과 짝 keyUp을 차단한다.
4. 같은 keyDown 처리 안에서 동일 포커스와 `원문 UTF-16 길이 + 이미 통과한 후행 마침표 길이` 커서를 확인한다.
...
교정 직후 6초 동안 같은 입력 필드와 예상 커서에 있고 다른 실제 키를 누르지 않았다면 `⌘Z`를 Mackor이 처리한다. exact range 좌표와 문자열 검증을 지원하는 앱에서는 첫 4초 동안 원문-only 칩 클릭으로 같은 복원 트랜잭션을 실행할 수 있다.
...
앱 샌드박스는 꺼져 있다. 공식 Developer ID Release는 Hardened Runtime을 켜지만, 현재 `install.sh`로 만든 ad-hoc 로컬 시험본은 이를 명시적으로 끈다. 전역 이벤트 탭이라는 제품 구조 때문에 일반 App Sandbox 앱과 같은 경계는 제공하지 않는다.
...
| 칩 표시·클릭 직전 교정된 exact range 문자열과 bounds | 주변 문장·필드 전체 값 |
| 최대 32개 물리 키코드·수정키·입력 간격 | 입력 단어·교정 결과의 파일/로그 저장 |
| 범위·대상 앱·기능 설정 | 입력 내용·교정 결과·대상 앱 목록의 네트워크 전송 |

exact range 문자열은 예상 replacement와 비교한 직후 버리고 로그·cache에 넣지 않는다. 입력 엔진에는 URLSession, Network.framework나 소켓 사용이 없다. Sparkle만 HTTPS appcast와 서명된 공식 배포물을 요청하며 시스템 프로파일링은 끈다. EventTap의 시작·중지·실패 상태만 콘솔에 출력하고 입력 내용은 출력하지 않는다.
...
- 전체 모드에서는 앱 추가 UI를 숨기고 “앱을 추가할 필요가 없음”을 표시한다.
- 선택 모드에서는 대상 앱과 앱별 두 기능을 하위 메뉴에서 관리한다.
- 전체 모드 상태 문구는 전역 한/영 자동 교정만 뜻하며, 전역 직접 조합을 암시하면 안 된다.
- exact range를 지원하는 앱의 교정 직후에는 물리 키열로 재구성한 원문 하나만 보이는 클릭 칩을 최대 4초 표시한다. 제목·“원문”·“되돌리기”·결과·화살표·단축키 안내는 넣지 않는다.
- 칩은 non-activating panel이며 실제 버튼 content rect만 hit-test한다. range 검증이나 좌표 조회가 실패하면 임의 위치에 띄우지 않고 기존 6초 `⌘Z`만 유지한다.
...
배포는 두 트랙을 함께 유지한다. GitHub 소스와 PR 기여 트랙은 Apple 공증이 필요 없으며, 사용자가 저장소를 신뢰하고 `install.command`로 직접 빌드·설치할 수 있다. 이 결과는 ad-hoc·미공증이고 공식 Sparkle 업데이트가 비활성이다. PR이 병합되면 관리자만 해당 병합 커밋에서 새 공식 산출물을 만든다.
...
| `EventTapManagerTests` | 51 | 네이티브 IME 유지, 기간 유예 상태기계, `BoundarySequence`, 제출 키 동기 순서·후행 마침표, 시스템 사전 경로의 사후 교정·충돌 보존, 안전 필드, 포커스·커서, 원문 칩 transaction, Undo, autorepeat |
...
1. **공개 가능한 Developer ID 산출물이 아직 없다.** 현재 설치본은 strict 검증을 통과하는 ad-hoc 로컬 개발본이고 legacy dist 산출물은 서명이 무효다. 키체인에는 유효한 코드서명 identity가 0개로 평가되므로 Developer ID 인증서·개인 키·신뢰 체인을 정상화하고 새 빌드로 앱·PKG·DMG의 서명, Gatekeeper 평가, 공증, staple을 모두 다시 검증해야 한다.
...
1. **실제 EventTap 지연과 부분 교체 위험:** Space·문장부호는 keyUp 뒤 AX 정착을 20ms 간격으로 최대 세 번 재확인한다. 제출 경계는 다음 키의 추월을 막기 위해 keyDown 처리 안에서 동기 교체하며 최대 32번의 Backspace 사이에 3ms를 둔다. 느리거나 멈춘 앱에서 이벤트 탭 timeout, 중간 포커스 변경 또는 일부만 지워진 상태가 가능한지 실측해야 한다.
...
3. **직접 조합과 보안 필드:** 선택 앱의 직접 조합 기능은 자동 교정 안전 게이트와 별도다. 비밀번호·검색·주소 필드에서도 조합을 반드시 막아야 하는지 결정하고 테스트해야 한다.
...
5. **메타데이터 힌트 휴리스틱:** 주소·검색·암호 필드 이름이 다른 언어이거나 커스텀 AX 메타데이터를 쓰면 분류가 달라질 수 있다.
...
7. **선택 앱 직접 조합의 네이티브 IME 기능 차이:** `전체 Mac`은 네이티브 IME를 유지하지만, 선택 앱에서 명시적으로 켠 직접 조합은 Apple IME pre-edit가 아니라 Backspace와 완성형 Unicode 주입이다. 조합 밑줄, 후보창, 한자 변환과 앱 Undo 단위가 네이티브 IME와 같다고 보장하지 않는다.
...
- [ ] 메뉴바 라벨이 `Mackor`이다.
- [ ] `전체 Mac` 선택 시 대상 앱 목록과 앱 추가 버튼이 숨는다.
- [ ] TextEdit, Safari 일반 본문 입력란, Chrome 일반 본문 입력란에서 별도 앱 등록 없이 동작한다.
- [ ] `전체 Mac`의 한국어 입력에서 물리 글자 keyDown/keyUp을 막거나 직접 Unicode 조합하지 않고 macOS 네이티브 IME가 조합한다.
- [ ] ABC/U.S.에서 `gksrmf `가 `한글 `로 바뀌고 시스템 입력 소스가 한국어 두벌식이 된다.
- [ ] 한국어 두벌식에서 `hello` 물리 키와 공백을 입력하면 `hello `가 되고 시스템 입력 소스가 이전 ABC/U.S.로 간다.
- [ ] 양방향 모두 경계의 물리 keyDown/keyUp이 통과하고, keyUp 뒤 사후 교체에서도 공백·문장부호가 정확히 하나만 남는다.
- [ ] `dho `가 `왜 `로 바뀐다.
- [ ] `dksehlsmsrjsep?`가 `안되는건데?`가 되며 `?`가 하나만 남는다.
- [ ] ABC/U.S.에서 `dkssudgktpdy` + Return·숫자패드 Enter·Tab이 각각 `안녕하세요`로 먼저 교정된 뒤 원래 제출·포커스 이동 동작을 정확히 한 번 수행한다.
...
- [ ] `ASD`, `HTTP` 같은 Latin-source 물리 ALL CAPS는 약어로 보존한다. Caps Lock을 켠 채 소문자 의도로 친 토큰도 화면과 같은 대소문자로 남는다.
...
- [ ] `worn/재구`처럼 macOS 양쪽 사전이 hit인 충돌은 텍스트·입력 소스·원문 칩을 모두 건드리지 않는다.
- [ ] `see/add/coffee/nth`처럼 부분 반복 자모·철자 휴리스틱 예외도 macOS 영어 hit + 한국어 miss이면 같은 일반 규칙으로 복구된다.
- [ ] 2타 `go/to/in`은 현재 자동교정 최소 길이 미만으로 보존된다.
- [ ] Shift가 필요한 후보는 현재의 보수적 Shift-sensitive veto 정책을 따른다.
- [ ] 마침표 1~3개 뒤 Space 등 최종 경계에서는 마침표가 정확히 보존되고, 마침표 뒤 내부 문자가 이어지거나 4번째 마침표가 오면 전체 run을 교정하지 않는다.
- [ ] exact range를 지원하는 입력란에서는 원문 하나만 보이는 칩이 교정 range에 표시되고 4초 뒤 사라지며, 칩이 없어져도 6초 안의 `⌘Z` 복원은 유지된다.
...
- [ ] Safari/Chrome 주소창과 검색창에서 기록·삭제·주입·입력 소스 전환이 없다.
...
- [ ] 등록 앱의 자동 교정만 켜면 조합 보정도 함께 켜진다.
- [ ] 조합 보정을 끄면 그 앱의 자동 교정도 꺼진다.
...
4. 보안/주소/검색 필드 차단이 자동 교정뿐 아니라 제품 전체의 약속과 일치하는가?
...
8. 내장/시스템 언어 자원과 점수식이 정상 코드·식별자·짧은 단어를 오교정하는 반례가 있는가?
9. 전체 모드에서 네이티브 IME의 marked text를 경계 keyDown으로 확정한 뒤 keyUp 이후 20ms 간격 최대 세 번의 정착 시점에 사후 교체하는 설계가 편집기별 AX 갱신 순서와 충돌하지 않는가?
...
특히 다음을 집중적으로 찾아 주세요.
1) 보안·주소·검색 필드로 입력이 새거나 수정되는 경로
