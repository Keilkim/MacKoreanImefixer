...
- **`Eh`류는 현재 파이프라인에서 이미 전부 보존된다.** `Eh/eh/ah/go/do/so/to`... 17개
  ...
  나타나는 함정.**
- ~~"지금은 단음절을 아예 안 건드리는 게 설계다"~~ — **이 문장은 틀렸다(2026-07-23 정정).**
  2키만 그렇고, **3키+ 단음절 435개는 오늘도 교정되고 있다.** 그중 265개가 코퍼스 빈도 0인
  음절이고, `cma`→츰 같은 파괴가 그 안에 살아 있었다. 무모음 게이트가 거부하는 1,208개
  중에는 영어 사전 단어가 **0개**여서, 그 게이트는 안전 장치가 아니라 방향이 반대인 대리
  지표였다. 전말은 `MONOSYLLABLE_CORRECTION.md` §2.
...
> **2026-07-23 정정 — 403/18은 재현되지 않는다.** 분모·분자의 정의가 적혀 있지 않아
> 세 가지 계수 방식을 모두 시도했으나 어느 것도 그 숫자를 내지 않았다(실측):
>
> | 계수 방식 | 2키 단음절 | 영어 단어(대소문자 무시) |
> |---|---:|---:|
> | 소문자 26자 | 168 | 37 |
> | 소문자 + 의미 있는 시프트 7자 | 240 | 52 |
> | `a-zA-Z` 52자를 별개 키열로 | 631 | 127 |
>
> 어느 정의를 쓰든 **결론의 방향은 같고 오히려 더 강하다** — 게이트를 그냥 열면
> 오변환률이 4.5%가 아니라 **20% 안팎**이다. 기저 영어 단어는 36종
> (`ab ah ai ak al am an ao ay di do eh el em en eu ey fi fo fu gi go ro sh si so th ti
> to tu vu wi wo wu wy xi zo`)이고, 인용된 `ah eh sh th`가 전부 그 안에 있다.
> 이 경고는 유효했고, 그래서 최종 설계는 게이트를 **열지 않고** 자산으로 열거한다.
> 앞으로 이런 수치를 적을 때는 반드시 계수 정의와 재현 명령을 함께 남긴다.
...
**D. ~~R-Short(`Eh↔또`, 아/이/그)는 별개·미측정 문제로 분리·보류.~~** — **해소됨.**
정본은 `MONOSYLLABLE_CORRECTION.md`.

보류 사유 두 개가 모두 성립하지 않는 것으로 드러났다.

1. *"`ko_words.txt`에 단음절이 0개라 코퍼스가 이 영역을 못 잰다"* — 사실이다(직접 재확인).
   그러나 결론이 틀렸다. 단음절 도메인은 **유한하고 구성적으로 전수 열거된다** — 초성19 ×
   중성21 × 종성28에서 현대 음절이 되는 키열이 정확히 **11,172개**다. 답은 "측정 불가 ⇒
   보류"가 아니라 **"측정 불가 ⇒ 열거"**였다. 표본이 아니라 전수이므로 코퍼스가 필요 없다.
2. *"건드리려면 `K_strong` 완화 → 새 위험"* — **게이트를 완화하지 않았다.** R-K4는 그대로고,
   동결 밖 계층이 해시로 동결된 자산에 있는 키열만 승격한다. R4-1 무접촉이다.

측정 결과(전부 실측):

| | 키열 수 |
|---|---:|
| 현대 한글 단음절이 되는 키열 전체 | 11,172 |
| 그중 동결 엔진이 `.correct` | 1,643 |
| 그중 **오늘 실제로 교정 중**(유모음) | 435 |
| 그중 무모음 게이트가 거부 | 1,208 (영어 사전 단어 **0개**) |
| 435개 중 코퍼스 빈도 0인 음절 | 265 |
| 최종 자산 `ko-mono.v1.txt` | **511** = 오늘 유지 422 + 신규 79 + 승격 10 |

이 과정에서 **오늘 살아 있는 파괴적 오교정 3건**(`cma`→츰, `aht`→못, `dha`→옴)이
드러나 함께 닫혔다. `Eh`(또)는 영어 사전 표제어라 게이트가 거부하므로 여전히 보존된다 —
그때의 우려가 옳았고, 이제 그 판단이 손 규칙이 아니라 사전 투영에서 자동으로 나온다.
...
8. ~~**단음절 자산 계층**(D 항목)~~ — **완료** (2026-07-23).
   `MonosyllableLexicon.swift`, `scripts/lexicon/ko-mono.v1.txt`(511행),
   `Corpus/lexical/v1/monosyllable.tsv` parity. 무모음 휴리스틱을 동결 자산 관문으로
   교체하고 2키 구제를 배선했다. 설계 기록은 `MONOSYLLABLE_CORRECTION.md`.
   후속 과제: 단음절 한국어 코퍼스 확보(KEEP 422행 중 262행이 빈도 0 음절이라 감사 불가),
   2키 토큰의 AX cold-start 대응(3번 항목과 같은 뿌리)
