...
- Corel이 `handle()==true` 소비를 무시한다 → **미확정** (위 ②)
- Corel에서 직접 조합(setMarkedText, NSNotFound) 가능 여부 → **미측정**
- Corel에서 AX 게이트가 실패해 R3 기록이 안 된다 → **추론** (직접 AX 측정은 Chrome뿐)
- 필드 42 마커의 NSEvent 왕복 생존 (A-1) → **미측정** (계측 수정 후 로그 0건)
- "하이브리드가 강제된다"는 결론 자체가 G0 결과에 조건부다. 단, Chrome AX nil은
  확정이므로 **IMK 도입 필요성은 불변** — G0가 뒤집을 수 있는 것은 "Corel에 탭이
  필요한가"뿐이고, 뒤집히면 계획이 단순해질 뿐이다(전 앱 IMK).

---

## 1. G0 — 재측정 팩 (선행 하드 게이트, 반나절)
...
3. **Corel AX 직접 측정**: `FocusedInputSafety`와 동일한 AX 질의를 Corel 텍스트 도구에
   직접 수행 (§6-5 추론의 검증)
4. **A-1**: 현행 Mackor 탭 주입 + 프로브 동시 가동, TextEdit·Corel에서
   `rawUserData=1212632134` 관측 여부 (정정된 기대값)
...
게이트: 1이 "소비 존중"으로 나오면 Corel도 IMK 후보 → 계획 단순화 분기.
"소비 무시" 확정이면 현 하이브리드 그대로.
...
- A-1 실패(마커 미생존) 시: 카운터+데드라인만으로는 부족 — **명시적 injection
  drain/handoff barrier**(탭 출력 완료 확인 후에만 owner 전이)를 Phase C에 넣는다.
  이 경우 A-1은 심층 방어 측정이 아니라 **Phase D+E 하드 게이트**다.
